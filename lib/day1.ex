...
""",
  ...
  def sum_groups_with_index(groups) do
    groups
    |> Enum.with_index()
    |> Enum.map(fn {group, index} -> %{index+1 => sum_group(group)} end)
  end
  ...
  def max_in_list_of_maps(list_of_maps) do
    list_of_maps
  ...
  def part2(input) do
    ...
    |> input_to_groups()
    |> sum_groups_with_index()
    |> sum_top3_in_list_of_maps()
  end

  def sum_top3_in_list_of_maps(list_of_maps) do
    list_of_maps
    |> Enum.map(fn map -> Map.values(map) end)
    |> List.flatten()
    |> Enum.sort()
